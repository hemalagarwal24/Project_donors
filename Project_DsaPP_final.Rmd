...
length(Exciting_project)/ length(data_outcomes$is_exciting)
 #0.05927411
```
Even though about 70% of the projects are fully funded, only % of the total projects can be deemed exciting from a business stand point. 
...
prop
# 0.2374802
#about 23% of the total projects(ignoring the projects for which the information is not available), are atleast one teacher referred donor
```
teachRefFunded <-c()
for(i in 1:length(data_outcomes$projectid)){
  if ((data_outcomes$at_least_1_teacher_referred_donor[i] == "t")&& (data_outcomes$fully_funded[i]=="t")){
    teachRefFunded[i] <-"t"
  }
}
...
Are schools with higher poverty levels likely to get funded any more than schools with lower poverty levels. 
```{r}
...
greater proportion of projects in urban areas are fully funded as compared to rural areas. Does the location of the school really matter to the donors? 
...
#0.1900954
```
Most of the projects come from specific states.Infact 48% of the total projects are posted from 5 states of the 52 :CA, NY, NC,IL, TX, and 19% come from CA alone. 
...
#0.2214089
```
About 53% of the donations (in terms of transcactions) come from 5 states.we also observe that ecept for FL, the states that make makinum number of donations are also the ones that post maximum number of projects. 
